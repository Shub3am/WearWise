...
func TestDecodeRejectsInvalidSleepSession(t *testing.T) {
	for _, testCase := range []struct {
		...
		{"stage starts before year 0 in UTC", map[string]any{"startAt": "0000-01-01T00:30:00+01:00", "stages": []any{
			map[string]any{"stage": "light", "startAt": "0000-01-01T00:30:00+01:00", "endAt": "2026-09-22T18:00:00Z"},
		}}, `sleepSessions[0].stages[0].startAt: must fall in years 0 to 9999 in UTC`},
		{"stage ends after year 9999 in UTC", map[string]any{"endAt": "9999-12-31T23:30:00-01:00", "stages": []any{
			map[string]any{"stage": "light", "startAt": "2026-09-22T17:00:00Z", "endAt": "9999-12-31T23:30:00-01:00"},
		}}, `sleepSessions[0].stages[0].endAt: must fall in years 0 to 9999 in UTC`},
...
func TestDecodeAcceptsSessionWithoutStagesPastYear9999InUTC(t *testing.T) {
	session := withFields(validSleepSession(), map[string]any{"endAt": "9999-12-31T23:30:00-01:00", "stages": nil})
	if _, err := samplebatch.Decode(batchBody(t, nil, []map[string]any{session}), decodeNow); err != nil {
		t.Fatal(err)
	}
}

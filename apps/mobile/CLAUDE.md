...
Entry points: `index.ts` (the app entry: sets up background sync, then loads expo-router) and `src/app/` (expo-router routes; `_layout.tsx` is the root). Environment: `EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY`, `EXPO_PUBLIC_API_URL`, `EXPO_PUBLIC_INGEST_URL` (see `.env.example`; Expo inlines them at build time).
